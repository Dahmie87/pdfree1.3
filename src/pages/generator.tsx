...
const CLIENT_VERSION = import.meta.env.VITE_PDFREE_CLIENT_VERSION as string | undefined;
...
const GeneratorPage = () => {
  ...
  const persistGeneratedBooks = (books: GeneratedBook[]) =>
    books.map(({ objectUrl, ...book }) => book);
  ...
  const normalizeLengthPriority = (value: string) => value.trim().replace(/\s+/g, '_');

  const stripPdfExtension = (name: string) => name.trim().replace(/\.pdf$/i, '');
  ...
  const handleGenerateBook = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const finalFilename = filenameInput.trim() ? ensurePdfExtension(stripPdfExtension(filenameInput.trim())) : filename;
      const finalTitle =
        useFilenameAsTitle && filenameInput.trim()
          ? stripPdfExtension(filenameInput.trim())
          : '';
      ...
      localStorage.setItem('generatedBooks', JSON.stringify(persistGeneratedBooks(updatedBooks)));
  ...
  const handleDeleteBook = (bookId: string) => {
    const bookToDelete = generatedBooks.find((book) => book.id === bookId);
    if (bookToDelete?.objectUrl) {
      window.URL.revokeObjectURL(bookToDelete.objectUrl);
    }

    const updatedBooks = generatedBooks.filter((book) => book.id !== bookId);
    setGeneratedBooks(updatedBooks);
    localStorage.setItem('generatedBooks', JSON.stringify(persistGeneratedBooks(updatedBooks)));
  };
                  ...
                  <div className="space-y-3">
                    {generatedBooks.map((book) => (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                        className="group rounded-xl border border-stone-200 bg-stone-50 p-3.5 transition-all duration-200 hover:shadow-md"
                      >
                        ...
                        <div className="max-h-0 overflow-hidden opacity-0 transition-all duration-200 group-hover:mt-3 group-hover:max-h-20 group-hover:opacity-100">
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              disabled={!book.objectUrl}
                              onClick={() => {
                                if (!book.objectUrl) return;
                                const downloadLink = document.createElement('a');
                                downloadLink.href = book.objectUrl;
                                downloadLink.download = book.filename;
                                downloadLink.click();
                              }}
                              className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-stone-900 px-3 py-2 text-xs font-bold text-white transition hover:bg-stone-800 disabled:cursor-not-allowed disabled:opacity-40"
                            >
                            ...
                            <button
                              ...
                              className="inline-flex items-center justify-center rounded-lg border border-stone-300 bg-white px-3 py-2 text-xs font-bold text-stone-700 transition hover:border-red-300 hover:text-red-700"
                            >
                              Delete
                            </button>
                          </div>
